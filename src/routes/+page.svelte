...
<Card.Root>
    ...
    <Card.Content class="flex flex-col gap-4">
        <div class="w-full h-max break-all p-4 bg-gray-50 rounded-md border-2 border-gray-100 relative">
            <span class="whitespace-pre-line">
                {output}
            </span>
            <Button class="absolute right-1 bottom-1 opacity-25 hover:opacity-75" size="icon" variant="ghost">
                <CopyIcon />
            </Button>
        </div>
        <div class="flex flex-col gap-8 w-full max-w-screen-2xl mx-auto">
                ...
                <div class="flex flex-col gap-2">
                    {#if item.type === PalworldConfigType.TextInput}
                        {#if item instanceof PalworldString}
                                ...
                                <Input id={key} disabled={item.readonly} maxlength={item.max} bind:value={item.value} />
                    ...
                    {:else if item.type === PalworldConfigType.Decimal}
                        {#if item instanceof PalworldDecimal}
                                <Label for={key}>{key}</Label>
                                <Slider id={key} value={[item.value]} disabled={item.readonly} min={item.min} max={item.max} step={item.step} onValueChange={(e) => {
                                    if (item instanceof PalworldDecimal) {
                                        item.value = e[0];
                                    }
                                }} />
                    ...
                    {:else if item.type === PalworldConfigType.SelectOne}
                        {#if item instanceof PalworldSelectOne}
                                ...
                                <Select.Root selected={item.value} disabled={item.readonly} onSelectedChange={(e) => item.value = e}>
                                    <Select.Trigger>
                    ...
                    {:else if item.type === PalworldConfigType.Number}
                        {#if item instanceof PalworldInteger}
                                <Label for={key}>{key}</Label>
                                <Slider id={key} value={[item.value]} disabled={item.readonly} min={item.min} max={item.max} step={1} onValueChange={(e) => {
                                    if (item instanceof PalworldInteger) {
                                        item.value = e[0];
                                    }
                                }} />
                    ...
                    {:else if item.type === PalworldConfigType.IpAddress}
                        {#if item instanceof PalworldIpAddress}
                                <Label for={key}>{key}</Label>
                                <Input id={key} disabled={item.readonly} required={item.required} bind:value={item.value} />
                        {/if}
                    {:else if item.type === PalworldConfigType.Port}
                        {#if item instanceof PalworldPort}
                                <Label for={key}>{key}</Label>
                                <Input id={key} type="number" disabled={item.readonly} required={item.required} min={item.min} max={item.max} bind:value={item.value} />
                        {/if}
                    {:else if item.type === PalworldConfigType.Url}
                        {#if item instanceof PalworldUrl}
                                <Label for={key}>{key}</Label>
                                <Input id={key} type="url" disabled={item.readonly} required={item.required} max={item.max} bind:value={item.value} />
                        {/if}
                    {/if}
                    {#if item.notice}
                        <p class="border-l-4 pl-2 border-l-yellow-300 text-yellow-600">{item.notice}</p>
                    {/if}
                </div>
            {/each}
        </div>
